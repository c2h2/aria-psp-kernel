...
static int goodix_request_irq(struct goodix_ts_data *ts)
{
	...
	ts->ct_irq_num = gpio_to_irq(ts->gpiod_int);

	err = request_threaded_irq(ts->ct_irq_num, NULL, goodix_ts_irq_handler,
		ts->irq_flags, "Goodix", ts);
	if (err < 0)
	{
		dev_err(&ts->client->dev, "%s: request IRQ failed!\n", __func__);
		free_irq(ts->ct_irq_num, ts);
	}
...
static int goodix_get_gpio_config(struct goodix_ts_data *ts)
{
	...
	gpio_export(gpiod, true);

	ts->gpiod_int = gpiod;

	gpiod = GPIO_TO_PIN(3, 20);
	error = gpio_request(gpiod, "Goodix");
	if (error < 0)
	{
		dev_dbg(dev, "Failed to get %d GPIO: %d\n",
